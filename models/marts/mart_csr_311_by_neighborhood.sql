-- 311 request density by SPD MCPP neighborhood. Requests carry only lat/lon, so
-- each is assigned to a neighborhood by point-in-polygon (spatial extension),
-- then normalized by area. The bbox prefilter keeps ST_Contains to ~one
-- candidate polygon per point. See mart_crime_by_neighborhood.
with mcpp as (
