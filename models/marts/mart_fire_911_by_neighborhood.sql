-- Fire 911 dispatch density by SPD MCPP neighborhood. Fire calls carry only
-- lat/lon, so each call is assigned to a neighborhood by point-in-polygon
-- (spatial extension), then normalized by area. The bbox prefilter keeps
-- ST_Contains to ~one candidate polygon per point. See mart_crime_by_neighborhood.
with mcpp as (
